...
df_steps = pd.DataFrame.from_records(
    metadata["dataQualityInfo"]["lineage"]["processStep"]
...
validated_compressed = pd.read_parquet(
    validate_compressed_files_parquet, engine="pyarrow"
)
...
reconstructed_features = pd.read_parquet(
    reconstructed_features_parquet, engine="pyarrow"
)
...
count_reconstructed_building = (
    count_reconstruction_input_building - count_not_reconstructed_building
)
count_reconstructed_buildingpart = reconstructed_features.loc[
    (yes_lod12 & yes_lod13 & yes_lod22 & (reconstructed_features.id.str.len() == 32)),
    "id",
].count()
...
invalid_cj_buildingpart_lod12 = (
    validated_compressed.cj_nr_invalid_buildingpart_lod12.sum().astype(int)
)
invalid_cj_buildingpart_lod13 = (
    validated_compressed.cj_nr_invalid_buildingpart_lod13.sum().astype(int)
)
invalid_cj_buildingpart_lod22 = (
    validated_compressed.cj_nr_invalid_buildingpart_lod22.sum().astype(int)
)
invalid_cj_buildingpart_lod12_pct = round(
    invalid_cj_buildingpart_lod12 / total_cj_buildingpart * 100, ROUNDING
)
invalid_cj_buildingpart_lod13_pct = round(
    invalid_cj_buildingpart_lod13 / total_cj_buildingpart * 100, ROUNDING
)
invalid_cj_buildingpart_lod22_pct = round(
    invalid_cj_buildingpart_lod22 / total_cj_buildingpart * 100, ROUNDING
)
mismatch_error_cj_lod12 = validated_compressed.cj_nr_mismatch_errors_lod12.sum().astype(
    int
)
mismatch_error_cj_lod13 = validated_compressed.cj_nr_mismatch_errors_lod13.sum().astype(
    int
)
mismatch_error_cj_lod22 = validated_compressed.cj_nr_mismatch_errors_lod22.sum().astype(
    int
)
...
invalid_obj_buildingpart_lod12 = (
    validated_compressed.obj_nr_invalid_buildingpart_lod12.sum().astype(int)
)
invalid_obj_buildingpart_lod13 = (
    validated_compressed.obj_nr_invalid_buildingpart_lod13.sum().astype(int)
)
invalid_obj_buildingpart_lod22 = (
    validated_compressed.obj_nr_invalid_buildingpart_lod22.sum().astype(int)
)
invalid_obj_buildingpart_lod12_pct = round(
    invalid_obj_buildingpart_lod12 / total_obj_buildingpart * 100, ROUNDING
)
invalid_obj_buildingpart_lod13_pct = round(
    invalid_obj_buildingpart_lod13 / total_obj_buildingpart * 100, ROUNDING
)
invalid_obj_buildingpart_lod22_pct = round(
    invalid_obj_buildingpart_lod22 / total_obj_buildingpart * 100, ROUNDING
)
...
cj_invalid_zip_list = validated_compressed.loc[
    validated_compressed["cj_zip_ok"] == False, "tile_id"
].sort_values(ascending=True)
cj_invalid_zip = (
    "-" if len(cj_invalid_zip_list) == 0 else ", ".join(cj_invalid_zip_list)
)
cj_invalid_schema_list = validated_compressed.loc[
    validated_compressed["cj_schema_valid"] == False, "tile_id"
].sort_values(ascending=True)
cj_invalid_schema = (
    "-" if len(cj_invalid_schema_list) == 0 else ", ".join(cj_invalid_schema_list)
)
cj_incomplete_lod_list = validated_compressed.loc[
    validated_compressed["cj_lod"] != "['0', '1.2', '1.3', '2.2']", "tile_id"
].sort_values(ascending=True)
cj_incomplete_lod = (
    "-" if len(cj_incomplete_lod_list) == 0 else ", ".join(cj_incomplete_lod_list)
)
obj_reconstructed_missing = count_reconstructed_building - total_obj_building
obj_invalid_zip_list = validated_compressed.loc[
    validated_compressed["obj_zip_ok"] == False, "tile_id"
].sort_values(ascending=True)
obj_invalid_zip = (
    "-" if len(obj_invalid_zip_list) == 0 else ", ".join(obj_invalid_zip_list)
)
gpkg_reconstructed_missing = count_reconstructed_building - total_gpkg_building
gpkg_invalid_zip_list = validated_compressed.loc[
    validated_compressed["gpkg_zip_ok"] == False, "tile_id"
].sort_values(ascending=True)
gpkg_invalid_zip = (
    "-" if len(gpkg_invalid_zip_list) == 0 else ", ".join(gpkg_invalid_zip_list)
)
gpkg_invalid_file_list = validated_compressed.loc[
    validated_compressed["gpkg_file_ok"] == False, "tile_id"
].sort_values(ascending=True)
gpkg_invalid_file = (
    "-" if len(gpkg_invalid_file_list) == 0 else ", ".join(gpkg_invalid_file_list)
)
...
def get_error_description(error_code):
    """Map error code to descriptive name."""
    error_map = {
        0: "OK",
        1: "BUILDING_EXTRA_ATTRIBUTES",
        2: "BUILDING_MISSING_ATTRIBUTES",
        3: "SURFACE_EXTRA_ATTRIBUTES",
        4: "SURFACE_MISSING_ATTRIBUTES",
        5: "INCORRECT_DATA_TYPE",
        6: "INCORRECT_NULLABLE",
    }
    return error_map.get(error_code, f"UNKNOWN_ERROR_{error_code}")


def aggregate_errors(df, column_name: str, format_prefix: str):
    """
    Aggregate attribute errors from a DataFrame.

    Parameters:
    df (pd.DataFrame): DataFrame containing '*_attributes_with_errors' column

    Returns:
    pd.DataFrame: Aggregated table with attributes and their error codes/descriptions
    """

    # Dictionary to collect all unique error codes for each attribute
    attribute_errors = defaultdict(set)

    # Process each row in the DataFrame
    for _, row in df.iterrows():
        error_string = row[column_name]

        # Skip if the cell is NaN or empty
        if pd.isna(error_string) or error_string == "":
            continue

        try:
            # Parse the string dictionary using eval
            error_dict = eval(error_string)

            # Iterate through each attribute and its error codes
            for attribute_name, error_codes in error_dict.items():
                # Add all error codes for this attribute to our set
                for error_code in error_codes:
                    attribute_errors[attribute_name].add(error_code)

        except Exception as e:
            print(f"Warning: Could not parse error string: {e}")
            continue

    # Create the result DataFrame
    result_rows = []

    for attribute_name, error_codes in attribute_errors.items():
        # Sort error codes for consistent output
        sorted_error_codes = sorted(list(error_codes))

        # Create description strings for each error code
        error_descriptions = [
            get_error_description(code) for code in sorted_error_codes
        ]

        result_rows.append(
            {
                "attribute": attribute_name,
                f"{format_prefix} attribute errors": ", ".join(error_descriptions),
            }
        )

    # Create DataFrame and sort by attribute name
    result_df = pd.DataFrame(result_rows)
    result_df = result_df.sort_values("attribute").reset_index(drop=True)

    return result_df


attributes_aggregated_errors_gpkg = aggregate_errors(
    validated_compressed, "gpkg_attributes_with_errors", "GPKG"
)
attributes_aggregated_errors_cj = aggregate_errors(
    validated_compressed, "cj_attributes_with_errors", "CityJSON"
)
df_attributes_aggregated_errors = (
    attributes_aggregated_errors_cj.set_index("attribute")
    .join(attributes_aggregated_errors_gpkg.set_index("attribute"), how="outer")
    .reset_index()
)
...
rf_pw_fraction = pd.DataFrame(
    {
        "ahn3": reconstructed_features[
            reconstructed_features["b3_nodata_fractie_ahn3"].notna()
        ].b3_nodata_fractie_ahn3,
        "ahn4": reconstructed_features[
            reconstructed_features["b3_nodata_fractie_ahn4"].notna()
        ].b3_nodata_fractie_ahn4,
        "ahn5": reconstructed_features[
            reconstructed_features["b3_nodata_fractie_ahn5"].notna()
...
rf_pw_radius = pd.DataFrame(
    {
        "ahn3": reconstructed_features[
            reconstructed_features["b3_nodata_radius_ahn3"].notna()
        ].b3_nodata_radius_ahn3,
        "ahn4": reconstructed_features[
            reconstructed_features["b3_nodata_radius_ahn4"].notna()
        ].b3_nodata_radius_ahn4,
        "ahn5": reconstructed_features[
            reconstructed_features["b3_nodata_radius_ahn5"].notna()
...
rf_pw_density = pd.DataFrame(
    {
        "ahn3": reconstructed_features[
            reconstructed_features["b3_puntdichtheid_ahn3"].notna()
        ].b3_puntdichtheid_ahn3,
        "ahn4": reconstructed_features[
            reconstructed_features["b3_puntdichtheid_ahn4"].notna()
        ].b3_puntdichtheid_ahn4,
        "ahn5": reconstructed_features[
            reconstructed_features["b3_puntdichtheid_ahn5"].notna()
...
def plot_pc_version(rf_pw_bron_counts):
    return go.Figure(
        data=[
            go.Pie(
                labels=rf_pw_bron_counts.index,
                values=rf_pw_bron_counts.values,
                hole=PIE_HOLE,
            )
        ]
    )
...
def plot_pc_selection_reason(rf_pw_selectie_counts):
    return go.Figure(
        data=[
            go.Pie(
                labels=rf_pw_selectie_counts.index,
                values=rf_pw_selectie_counts.values,
                hole=PIE_HOLE,
            )
        ]
    )
...
def plot_pc_density(rf_pw_density):
    ...
    p.set_axis_labels(
        f"Point density [pt/m2]. Range limited to {x_range}. Binwidth={binwidth}"
    )
    p.legend.set_title("Point cloud")
    plt.xlim(*x_range)
    plt.savefig(buf, format="svg")
...
def plot_validity_cityjson_lod12(validated_compressed):
    _ae = itertools.chain.from_iterable(
        eval(e)
        for e in validated_compressed.cj_errors_lod12
...
def plot_validity_cityjson_lod13(validated_compressed):
    _ae = itertools.chain.from_iterable(
        eval(e)
        for e in validated_compressed.cj_errors_lod13
        if isinstance(e, str) and e != "[]"
    )
    all_errors = list(_ae)
    fig = go.Figure(data=[go.Pie(labels=all_errors, hole=PIE_HOLE)])
...
def plot_validity_cityjson_lod22(validated_compressed):
    _ae = itertools.chain.from_iterable(
        eval(e)
        for e in validated_compressed.cj_errors_lod22
        if isinstance(e, str) and e != "[]"
    )
    all_errors = list(_ae)
    fig = go.Figure(data=[go.Pie(labels=all_errors, hole=PIE_HOLE)])
...
def plot_validity_obj_lod12(validated_compressed):
    _ae = itertools.chain.from_iterable(
        eval(e)
        for e in validated_compressed.obj_errors_lod12
        if isinstance(e, str) and e != "[]"
    )
    all_errors = list(_ae)
...
def plot_validity_obj_lod13(validated_compressed):
    _ae = itertools.chain.from_iterable(
        eval(e)
        for e in validated_compressed.obj_errors_lod13
        if isinstance(e, str) and e != "[]"
    )
    all_errors = list(_ae)
    fig = go.Figure(data=[go.Pie(labels=all_errors, values=all_errors, hole=PIE_HOLE)])
...
def plot_validity_obj_lod22(validated_compressed):
    _ae = itertools.chain.from_iterable(
        eval(e)
        for e in validated_compressed.obj_errors_lod22
        if isinstance(e, str) and e != "[]"
    )
    all_errors = list(_ae)
    fig = go.Figure(data=[go.Pie(labels=all_errors, values=all_errors, hole=PIE_HOLE)])
...
card_pc_selection = dbc.Card(
    dbc.CardBody(
        [
            ...
            html.P(
                [
                    "Reason for the selected point cloud source. Attribute: ",
                    html.A(
...
app.layout = dbc.Container(
    [
        html.H3(
            f"3DBAG version {metadata['identificationInfo']['citation']['edition']}",
        ...
        dbc.Row(
            [
                dash_table.DataTable(
                    data=df_software.to_dict("records"),
        ...
        html.H3("Attribute errors", className="title is-3"),
        dbc.Row(
            [
                dash_table.DataTable(
                    data=df_attributes_aggregated_errors.to_dict("records"),
                    style_cell={"whiteSpace": "pre-line", "textAlign": "left"},
                ),
            ],
            align="center",
        ...
        dbc.Row(
            [
                dash_table.DataTable(
                    data=df_format_counts_cj.to_dict("records"),
                    style_cell={"whiteSpace": "pre-line", "textAlign": "left"},
                ),
            ],
            align="center",
        ...
        dbc.Row(
            [
                dash_table.DataTable(
                    data=df_val3dity_params.to_dict("records"),
                    style_cell={"whiteSpace": "pre-line", "textAlign": "left"},
                    columns=[
                        {
                            "name": "parameter",
